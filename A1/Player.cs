...
public partial class Player : CharacterBody3D
{
	...
	{
		distance = GlobalTransform.origin.DistanceTo(NodeThing.GlobalTransform.origin);
		...
		//GD.Print("Camera Position: ", cameraPosition);
		...
		//camera thingy
		float x = NodeThing.Position.X + Mathf.Cos(angle) * distance ;
		float z = NodeThing.Position.Z + Mathf.Sin(angle) * distance ;

		Position = new Vector3(x, Position.Y, z);
		LookAt(NodeThing.Position,Vector3.Up);
